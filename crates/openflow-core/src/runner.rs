...
/// What [`LOCAL_RUNNER_LOCK`] is compiled from, in order.
///
/// `pip` is handed the lock, not this: a `pip install mlx-audio==0.5.1` on the
/// command line would resolve the other 34 packages fresh on every machine.
/// This is the input that resolution starts from, and `scripts/lock-local-runner.sh`
/// reads the same two constants to rebuild it.
///
/// A function rather than a `const` because both entries are formatted, and one
/// list rather than two strings because the property that matters -- no bare
/// requirement, ever -- is one a test can only state about a whole list.
...
/// Every Python minor [`LocalRunner::find_python`] will pick, newest first.
///
/// Also the range [`LOCAL_RUNNER_LOCK`] is compiled across. The two have to be
/// the same list: a Python the runner will pick but the lock does not cover is
/// a machine where Install fails, and a Python the lock covers but the runner
/// never picks is dead weight in the file.
pub const SUPPORTED_PYTHONS: &[(u32, u32)] = &[(3, 14), (3, 13), (3, 12), (3, 11), (3, 10)];

/// The whole package set the venv gets, pinned, with the hash of every artifact
/// each package is allowed to come from.
///
/// [`pip_requirements`] pins what this module names. It does not pin what those
/// two packages *pull in*: `mlx-audio` 0.5.1 asks for `transformers>=5.14.0`,
/// `mlx>=0.31.1`, `numpy>=1.26.4` and five more, every one a floor with no
/// ceiling. The closure is 36 packages, and in the five days between the
/// benchmark in `docs/native-port/local-runner-benchmark.md` and this being
/// written, two of them shipped a new release. So the same build of this app,
/// installed a week apart, is not the same transcription backend.
///
/// Generated by `scripts/lock-local-runner.sh`; the tests below fail when it
/// and this module disagree. Note what it is *not*: a single version per
/// package. `numpy` and `scipy` have each dropped Pythons this runner still
/// supports, so a 3.10 venv resolves `numpy` 2.2.6 where a 3.12 one resolves
/// 2.5.3. The file carries all three, behind `python_full_version` markers.
pub const LOCAL_RUNNER_LOCK: &str = include_str!("../requirements/local-runner.txt");
...
fn pip_install_command(python: &Path, requirements: &Path) -> Command {
...
"#;
    ...
    /// One requirement line of [`LOCAL_RUNNER_LOCK`].
    struct Locked {
        name: String,
        version: String,
        marker: String,
    ...
    /// Read the lock the way `pip` reads it.
    ///
    /// A requirement starts in column zero; everything indented under it is
    /// that requirement's `--hash=` lines and `# via` annotations. Comment
    /// lines are the header. Nothing else is allowed to appear, which is what
    /// makes "every requirement is pinned" a statement about the whole file
    /// rather than about the lines this happened to recognise.
    fn locked_requirements() -> Vec<Locked> {
        let mut out: Vec<Locked> = Vec::new();
        for line in LOCAL_RUNNER_LOCK.lines() {
            let trimmed = line.trim_end().trim_end_matches('\\').trim_end();
            if trimmed.is_empty() || trimmed.starts_with('#') {
                continue;
            }
            if line.starts_with(char::is_whitespace) {
                let indented = trimmed.trim_start();
                if let Some(hash) = indented.strip_prefix("--hash=") {
                    let digest = hash
                        .strip_prefix("sha256:")
                        .unwrap_or_else(|| panic!("{indented} is not a sha256 hash"));
                    assert!(
                        digest.len() == 64 && digest.bytes().all(|byte| byte.is_ascii_hexdigit()),
                        "{indented} is not a 64-character hex digest"
                    );
                    out.last_mut()
                        .unwrap_or_else(|| panic!("{indented} belongs to no requirement"))
                        .hashes += 1;
                } else {
                    assert!(
                        indented.starts_with('#'),
                        "the lock has an indented line that is neither a hash nor an \
                         annotation, and pip would read it as something this test does \
                         not check: {indented}"
                    );
                }
                continue;
            }
            let (requirement, marker) = match trimmed.split_once(';') {
                Some((requirement, marker)) => (requirement.trim(), marker.trim()),
                None => (trimmed, ""),
            };
            let (name, version) = requirement
                .split_once("==")
                .unwrap_or_else(|| panic!("the lock does not pin {requirement}"));
            out.push(Locked {
                name: name.trim().to_ascii_lowercase().replace('_', "-"),
    ...
    /// Does a `python_full_version` marker admit this Python?
    ///
    /// Deliberately narrow: it understands the comparisons a resolver writes
    /// against `python_full_version` and nothing else. A marker it cannot read
    /// is a panic rather than a `false`, because the test it serves is a
    /// coverage test -- silently deciding "this entry does not apply" is
    /// exactly how such a test reports full coverage of nothing.
    fn python_marker_admits(marker: &str, python: (u32, u32)) -> bool {
        let rest = marker
            .strip_prefix("python_full_version")
            .unwrap_or_else(|| panic!("not a python_full_version marker: {marker}"))
            .trim();
        let (operator, literal) = ["<=", ">=", "==", "!=", "<", ">"]
            .iter()
            .find_map(|operator| rest.strip_prefix(*operator).map(|rest| (*operator, rest)))
            .unwrap_or_else(|| panic!("unreadable comparison in marker: {marker}"));
        let literal = literal.trim().trim_matches(|c| c == '\'' || c == '"');
        let wildcard = literal.ends_with(".*");
        let mut parts = literal.trim_end_matches(".*").split('.');
        let major: u32 = parts
            .next()
            .and_then(|p| p.parse().ok())
            .unwrap_or_else(|| {
                panic!("no major version in marker: {marker}");
            });
    ...
    ///
    /// The bare `huggingface_hub` this once replaced is the failure this test is
    ...
    ///
    /// The second half is what changed when the lock arrived: `pip` is handed
    /// the file and nothing else. A requirement left on the argv would be
    /// resolved rather than read out of the lock, and `--require-hashes` would
    /// then reject the whole install -- but only on a machine that got as far
    /// as running it, which is not this test.
    #[test]
    fn the_venv_is_built_from_exactly_pinned_requirements() {
        let requirements = pip_requirements();
        let argv: Vec<String> = pip_install_command(
            Path::new("/venv/bin/python"),
            Path::new("/app/runner/requirements.txt"),
        )
        .get_args()
        ...
        for requirement in &requirements {
            let (name, version) = requirement
                .split_once("==")
                .unwrap_or_else(|| panic!("the lock is compiled from unpinned {requirement}"));
            ...
            assert!(
                !argv.iter().any(|argument| argument.contains(name)),
                "pip is given {name} on the command line, so it resolves it instead \
                 of reading it out of the lock: {argv:?}"
            );
        }
        assert!(
            argv.iter().any(|argument| argument == "--require-hashes"),
            "pip would accept an artifact the lock does not name: {argv:?}"
        );
        let dash_r = argv
            .iter()
            .position(|argument| argument == "-r")
            .unwrap_or_else(|| panic!("pip is given no requirements file: {argv:?}"));
        assert_eq!(
            argv.get(dash_r + 1).map(String::as_str),
            Some("/app/runner/requirements.txt"),
            "-r does not name the file install_steps wrote: {argv:?}"
        );
    }

    /// Every package in the lock is pinned, and every pin carries a hash.
    ///
    /// `--require-hashes` makes pip refuse the whole file when one requirement
    /// is missing hashes, so this says nothing pip would not also say -- on a
    /// machine that runs pip. That is the point: the closure is 36 packages
    /// deep and nobody reviewing a regenerated lock reads all of it.
    #[test]
    fn every_package_in_the_lock_is_pinned_and_hashed() {
        let locked = locked_requirements();
        assert!(
            locked.len() > 30,
            "the lock resolved to {} packages, which is not a closure of mlx-audio",
            locked.len()
        );
        for entry in &locked {
            assert!(
            ...
            assert!(
                entry.hashes > 0,
                "{}=={} carries no hash, so pip would take whatever the index served",
                entry.name,
                entry.version
            );
        }
    ...
    /// The lock pins what this module asks for, and this module asks for what
    /// the lock was compiled from.
    ///
    /// This is the test that makes the two constants safe to edit. Moving
    /// `MLX_AUDIO_VERSION` without re-running `scripts/lock-local-runner.sh`
    /// leaves the app installing the old closure while claiming the new
    /// version, and that is a failure nothing else here would notice.
    #[test]
    fn the_lock_pins_the_versions_this_module_asks_for() {
        let locked = locked_requirements();
        for requirement in pip_requirements() {
            let (name, version) = requirement.split_once("==").expect("a pinned requirement");
            let name = name.to_ascii_lowercase().replace('_', "-");
            let found: Vec<&str> = locked
                .iter()
            ...
            assert_eq!(
                found,
                vec![version],
                "the lock and runner.rs disagree about {name}: re-run \
                 scripts/lock-local-runner.sh"
            );
        }
    }

    /// One resolution, for every Python the runner will pick.
    ///
    /// The reason this test exists rather than a single pin per package:
    /// `numpy` and `scipy` have each dropped Pythons this runner still
    /// supports, so the newest release that installs on 3.14 does not exist for
    /// 3.10. A package that appears more than once is answering that, and the
    /// thing that can go wrong is a gap -- a supported Python that matches no
    /// entry at all, which is an Install that fails on somebody's Mac and
    /// nowhere else.
    ///
    /// "Exactly one" rather than "at least one" because two matching entries is
    /// the same file telling pip two different versions, and which one it takes
    /// is not something this repository decides.
    #[test]
    fn the_lock_covers_every_python_the_runner_will_pick() {
        let locked = locked_requirements();
        let mut forked: std::collections::BTreeMap<&str, Vec<&Locked>> =
            std::collections::BTreeMap::new();
        for entry in &locked {
            forked.entry(entry.name.as_str()).or_default().push(entry);
        }
        let mut checked = 0;
        for (name, entries) in forked {
            if entries.len() == 1 {
                continue;
            }
            for entry in &entries {
                assert!(
                    entry.marker.starts_with("python_full_version"),
                    "{name} is split on something other than the Python version \
                     ({}), which this test does not evaluate -- extend it rather \
                     than letting it report coverage it did not check",
                    entry.marker
                );
            }
            for python in SUPPORTED_PYTHONS {
                let matching: Vec<&str> = entries
                    .iter()
                    .filter(|entry| python_marker_admits(&entry.marker, *python))
                    .map(|entry| entry.version.as_str())
                    .collect();
                assert_eq!(
                    matching.len(),
                    1,
                    "on Python {}.{}, {name} resolves to {matching:?} -- the lock \
        ...
        assert!(
            checked > 0,
            "no package in the lock is split by Python version, so this test \
             checked nothing -- if that is really true, delete it"
        );
    }

    /// The list the runner searches and the floor it advertises agree.
    ///
    /// `missing_python_message` tells the user to install a Python at least
    /// `MINIMUM_PYTHON`, `find_python` looks for the versions in
    /// `SUPPORTED_PYTHONS`, and the lock is compiled from the bottom of that
    /// list. Three places, one fact.
    #[test]
    fn the_searched_pythons_run_down_to_the_advertised_floor() {
        assert_eq!(
            SUPPORTED_PYTHONS.last().copied(),
            Some(MINIMUM_PYTHON),
            "the runner searches for a Python it tells the user is too old"
        );
        let mut sorted = SUPPORTED_PYTHONS.to_vec();
        sorted.sort_unstable();
        sorted.reverse();
        assert_eq!(
            sorted.as_slice(),
            SUPPORTED_PYTHONS,
            "find_python takes the first candidate that qualifies, so the list \
             has to run newest first"
        );
        assert!(
            LocalRunner::missing_python_message()
                .contains(&format!("{}.{}", MINIMUM_PYTHON.0, MINIMUM_PYTHON.1)),
            "the message names a different floor than the constant"
        );
    }
